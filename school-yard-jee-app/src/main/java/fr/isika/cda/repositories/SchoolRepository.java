...
public class SchoolRepository {
	...
	public School getSchoolById(Long id) {
		return entityManager.createQuery("SELECT s FROM School s WHERE s.id = :id_param", School.class)
	...
	public List<School> getByName(String name) {
		return entityManager
				...
				.setParameter("name_param", name).getResultList();
	}

	public List<School> getByType(SchoolTypeEnum schoolType) {
		return entityManager.createQuery("SELECT s FROM School s WHERE s.schoolTypeEnum = :type_param", School.class)
				.setParameter("type_param", schoolType).getResultList();
	}
